...
const UI={
  ...
  amb(){
    ...
    if(m===0||m===1||m===3){
      ...
      else if(m===1){const grads=[["#3b41d8","#6468f5"],["#c77b10","#f0aa22"],["#b8223b","#eb3a5f"],["#0e766e","#14b8a6"],["#7c3aed","#a855f7"]];for(let i=0;i<6;i++){const gc=grads[~~(r()*grads.length)],TW=256,TH=358,oc=C("canvas");oc.width=TW;oc.height=TH;const cx2=oc.getContext("2d");cx2.beginPath();cx2.roundRect(0,0,TW,TH,16);cx2.clip();const grd=cx2.createLinearGradient(0,0,TW,TH);grd.addColorStop(0,gc[0]);grd.addColorStop(1,gc[1]);cx2.fillStyle=grd;cx2.fillRect(0,0,TW,TH);cx2.strokeStyle="rgba(255,255,255,0.42)";cx2.lineWidth=3;cx2.stroke();const spr=new PIXI.Sprite(new PIXI.Texture(new PIXI.BaseTexture(oc)));spr.anchor.set(.5);spr.alpha=.68;a.stage.addChild(spr);const sz=(30+25*r())/100*window.innerWidth;spr.width=sz;spr.height=sz*1.4;o.push({spr,sx:(i%2+.1+r()*.8)/2*.9,sy:(Math.floor(i/2)+.1+r()*.8)/3*.9,tx:(20*r()-10)/100,ty:(20*r()-10)/100,r0:(40*r()-20)*Math.PI/180,r1:(40*r()-20)*Math.PI/180,d:(20+15*r())*1e3,ph:r()*Math.PI*2});}}
      else if(m===3){
      ...
      if(State.cfg.ambStatic)setTimeout(()=>a.ticker.stop(),m===3?350:50);
    }else if(m===2){for(let i=0;i<12;i++){const d=C("div");d.className="f-obj f-neon";const sz=8+18*r(),M=`rgb(${c[~~(r()*c.length)]})`,k=r()>.5?45:30*r()-15;d.style.cssText=`width:${sz}vw;height:${r()>.5?sz:sz*(.5+.8*r())}vw;top:${88*r()}%;left:${88*r()}%;`;d.style.setProperty("--glow",M);d.style.setProperty("--d",14+18*r()+"s");d.style.setProperty("--pd",2.5+2*r()+"s");d.style.setProperty("--tx",26*r()-13+"vw");d.style.setProperty("--ty",26*r()-13+"vh");d.style.setProperty("--r0",k+"deg");d.style.setProperty("--r1",k+60*r()-30+"deg");bg.appendChild(d)}if(State.cfg.ambStatic)requestAnimationFrame(()=>requestAnimationFrame(()=>bg.querySelectorAll('.f-obj').forEach(el=>el.style.animationPlayState='paused')));}
    else{
...
const Actions={
  ...
  if(!a)return;e.stopPropagation();const act=a.dataset.action,m={
    ...
    'save-file':()=>{const v="4.3.2 (Web)",b=new Blob([JSON.stringify({version:v,config:State.cfg,users:State.usr})],{type:"application/json"}),u=URL.createObjectURL(b),lk=C("a");lk.href=u;lk.download=`Mgo_Backup_V${v}.json`;D.body.appendChild(lk);lk.click();D.body.removeChild(lk);setTimeout(()=>URL.revokeObjectURL(u),5e3);UI.toast(T("file_dl"))},

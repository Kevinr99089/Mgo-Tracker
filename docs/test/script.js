try{const v=localStorage.getItem('mgo_unified_version');if(v)document.documentElement.classList.add(v+'-mode');const c=JSON.parse(localStorage.getItem('mgo_cfg'));if(c&&c.ambiance===4){const Rnd=(s)=>function(){var t=s+=1831565813;return t=Math.imul(t^t>>>15,1|t),(((t^=t+Math.imul(t^t>>>7,61|t))^t>>>14)>>>0)/4294967296};if(Rnd(c.seed||Date.now())()<0.01)document.documentElement.classList.add('is-shiney');}}catch(e){}
let i18n={};
const T=(k)=>i18n[k]||k,esc=(s)=>(s+"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;");
...
function tr(){Q("[data-i18n]").forEach(e=>{const k=e.dataset.i18n;k==="add_upper"?e.innerHTML=T(k):e.textContent=T(k)});Q("[data-i18n-html]").forEach(e=>{e.innerHTML=i18n[e.dataset.i18nHtml]||''||'';});document.title=i18n.page_title||''||'';}
...
function __playShineySplash(isFast = false) {
  ...
  setTimeout(() => {
      ...
      setTimeout(() => {
          if(spt) { spt.innerHTML = i18n.shiney_title||''; spt.style.opacity = "1"; }
          if(sph) { sph.textContent = i18n.shiney_hint||''; sph.style.opacity = "1"; }
      }, 250);
  }, isFast ? 150 : 600);
}
D.addEventListener("DOMContentLoaded",()=>{
  (function(){
    ...
    const map={
      'fr-ca':'fr.txt','fr-be':'fr.txt',
      'en-us':'en.txt','en-gb':'en.txt','en-au':'en.txt','en-ca':'en.txt','en-nz':'en.txt','en-za':'en.txt',
      'nl-nl':'nl.txt','nl-be':'nl.txt',
      'zh-cn':'zh.txt','zh-sg':'zh.txt','zh-hans':'zh.txt',
      'zh-tw':'zh-hant.txt','zh-hk':'zh-hant.txt','zh-mo':'zh-hant.txt','zh-hant':'zh-hant.txt',
      'ko-kr':'ko.txt',
      'ja-jp':'ja.txt',
      'ar-sa':'ar.txt','ar-eg':'ar.txt','ar-ma':'ar.txt','ar-dz':'ar.txt','ar-tn':'ar.txt','ar-ly':'ar.txt','ar-ae':'ar.txt',
      'pt-br':'pt.txt','pt-pt':'pt.txt',
      'ms-my':'ms.txt','ms-sg':'ms.txt','ms-bn':'ms.txt',
      'tr-tr':'tr.txt',
      'th-th':'th.txt'
    };
    const baseMap={
      'fr':'fr.txt','de':'de.txt','it':'it.txt','es':'es.txt','pt':'pt.txt',
      'nl':'nl.txt','pl':'pl.txt','tr':'tr.txt',
      'zh':'zh.txt','ja':'ja.txt','ko':'ko.txt','ar':'ar.txt',
      'ms':'ms.txt','th':'th.txt'
    };
    const primary=map[lang]||baseMap[base]||'en.txt';
    const loadJson=f=>fetch(f).then(r=>r.ok?r.json():Promise.reject());
    loadJson(primary)
      .catch(()=>primary==='en.txt'?Promise.reject():loadJson('en.txt'))
      .catch(()=>({}))
      .then(strings=>{
      i18n=strings;
      State.init();
  ...
  amb(){
    ...
    if(m===0||m===1||m===3){
      if(typeof PIXI==='undefined'){
        if(!window._pxErr){UI.toast(i18n.ambiance_error||'');window._pxErr=!0}
      ...
      else if(m===3){const BPAD=80;const lw=C("div");lw.style.cssText="position:absolute;inset:0;background:#020204;overflow:hidden;";bg.appendChild(lw);lw.appendChild(a.view);a.view.style.cssText=`position:absolute;top:-${BPAD}px;left:-${BPAD}px;pointer-events:none`;const _rsz=()=>{const W=bg.offsetWidth||window.innerWidth,H=bg.offsetHeight||window.innerHeight;a.renderer.resize(W+BPAD*2,H+BPAD*2)};_rsz();window.addEventListener('resize',_rsz);_pxApp=()=>{window.removeEventListener('resize',_rsz);a.destroy(!0,{children:!0,texture:!0,baseTexture:!0})};const goo=new PIXI.Container(),blur=new PIXI.BlurFilter(14,4);blur.padding=BPAD;const thresh=new PIXI.Filter(null,"varying vec2 vTextureCoord;uniform sampler2D uSampler;void main(){vec4 c=texture2D(uSampler,vTextureCoord);vec3 rgb=(c.a>0.001)?c.rgb/c.a:vec3(0.0);float a=clamp(c.a*22.0-9.0,0.0,1.0);gl_FragColor=vec4(rgb*a,a);}");thresh.padding=BPAD;goo.filters=[blur,thresh];a.stage.addChild(goo);const gfx=new PIXI.Graphics();goo.addChild(gfx);const P=2+~~(2*r()),D=[...c].sort(()=>r()-.5),pal=D.slice(0,P).map(x=>{const p=x.split(',');return (p[0]<<16)|(p[1]<<8)|p[2]}),blobs=[];for(let i=0;i<10;i++){const ny=r();blobs.push({x:r()*(a.screen.width-BPAD*2||window.innerWidth),y:ny*(a.screen.height-BPAD*2||window.innerHeight),vx:0,vy:0,r:35+55*r(),baseR:35+55*r(),col:pal[~~(r()*pal.length)],ph:r()*Math.PI*2,fq:.08+.12*r(),df:.06*(r()-.5),tp:1-ny,ti:.12+.18*r()})}let lt=0;a.ticker.add(()=>{const n=performance.now();const ds=Math.min((n-lt)/1e3,.05);if(!lt){lt=n;return}lt=n;const W=a.screen.width-BPAD*2,H=a.screen.height-BPAD*2;for(let i=0;i<10;i++){const b=blobs[i];b.tp+=(b.y/H-b.tp)*b.ti*ds;b.vy-=(b.tp-.42)*6.5*ds;b.ph+=b.fq*ds;b.vx+=Math.sin(b.ph+1.7*i)*ds+b.df*ds*3;for(let j=i+1;j<10;j++){const b2=blobs[j],dx=b2.x-b.x,dy=b2.y-b.y,d=Math.sqrt(dx*dx+dy*dy)+1,mn=.35*(b.r+b2.r);if(d<mn){const f=.08*(mn-d)*ds,nx=dx/d,ny=dy/d;b.vx-=nx*f;b.vy-=ny*f;b2.vx+=nx*f;b2.vy+=ny*f}}b.vx*=.994;b.vy*=.994;b.x+=b.vx;b.y+=b.vy;const pd=.3*b.r;if(b.x<-pd||b.x>W+pd){b.vx=Math.abs(b.vx)*.3*(b.x<-pd?1:-1);b.x=b.x<-pd?-pd:W+pd}if(b.y<-pd||b.y>H+pd){b.vy=Math.abs(b.vy)*.3*(b.y<-pd?1:-1);b.y=b.y<-pd?-pd:H+pd}b.r=b.baseR+Math.sin(6e-4*n+2.1*i)*b.baseR*.06}gfx.clear();for(let i=0;i<10;i++){gfx.beginFill(blobs[i].col,.92);gfx.drawCircle(blobs[i].x+BPAD,blobs[i].y+BPAD,blobs[i].r*1.8);gfx.endFill();}});}
      if(m!==3)_pxApp=()=>a.destroy(!0,{children:!0,texture:!0,baseTexture:!0})
...
const Share={
  ...
  async quick(n,id,btn){
    ...
    const fb=()=>{const t=C("textarea");t.value=url;t.style.position="fixed";t.style.opacity="0";D.body.appendChild(t);t.focus();t.select();try{if(D.execCommand("copy"))scc();else throw new Error()}catch(e){prompt(i18n.share_link_prompt||'',url);btn.style.pointerEvents="auto"}t.remove()};
  ...
  show(){const s=this._pi;if(!s)return;E("import-name").textContent=i18n.user_prefix||''+s.name;const c=Object.values(s.data.state||{}).filter(x=>x===1).length,d=Object.values(s.data.state||{}).filter(x=>x===2).length;E("import-stats").textContent=`${c} ${i18n.share_cards||''} · ${d} ${i18n.share_dupes||''}`;const match=State.cfg.usersList.includes(s.name);let mem={};try{mem=JSON.parse(LS.getItem("mgo_share_mem")||"{}")}catch(e){}const rem=mem[s.name],valid=rem&&State.cfg.usersList.includes(rem);const bc=E("btn-import-confirm"),br=E("btn-import-replace"),bq=E("btn-import-quick");if(match){bc.textContent=(i18n.import_update_existing||'').replace('{name}',s.name);br.style.display="none";bq.style.display="none";this._qt=null}else{bc.textContent=i18n.import_add_btn||'';br.style.display="";if(valid){bq.style.display="";bq.textContent=(i18n.import_update_quick||'').replace('{name}',rem);this._qt=rem}else{bq.style.display="none";this._qt=null}}E("import-step-1").style.display="flex";E("import-step-2").style.display="none";E("mod-import").classList.add("open")},
  ...
  cI(){if(!this._pi)return;const t=this._pi.name.replace(/\s/g,"");if(!State.cfg.usersList.includes(this._pi.name)){State.cfg.usersList.push(this._pi.name);State.saveC()}State.usr[t]={state:{},nums:{},...this._pi.data};State.saveU(t);UI.toast((i18n.toast_imported||'').replace('{name}',this._pi.name));this._pi=null;this.cls();setTimeout(()=>location.reload(),900)},
  oR(){const l=E("import-player-select");l.innerHTML="";this._rt=null;E("btn-import-replace-confirm").disabled=!0;State.cfg.usersList.forEach(n=>{const b=C("button");b.className="mini-btn sb-btn";b.textContent=i18n.user_prefix||''+n;b.onclick=()=>{l.querySelectorAll(".mini-btn").forEach(el=>{el.style.background="";el.style.borderColor="";el.style.color=""});b.style.background="var(--p)";b.style.borderColor="var(--p)";b.style.color="#fff";this._rt=n;E("btn-import-replace-confirm").disabled=!1};l.appendChild(b)});E("import-step-1").style.display="none";E("import-step-2").style.display="flex"},
  cR(){if(!this._pi||!this._rt)return;if(!confirm((i18n.confirm_replace||'').replace('{name}',this._rt)))return;const t=this._rt.replace(/\s/g,"");State.usr[t]={state:{},nums:{},...this._pi.data};State.saveU(t);try{const m=JSON.parse(LS.getItem("mgo_share_mem")||"{}");m[this._pi.name]=this._rt;LS.setItem("mgo_share_mem",JSON.stringify(m))}catch(e){}UI.toast((i18n.toast_replaced||'').replace('{name}',this._rt));this._pi=null;this._rt=null;this.cls();setTimeout(()=>location.reload(),900)},
  cQ(){if(!this._pi||!this._qt)return;const t=this._qt.replace(/\s/g,"");State.usr[t]={state:{},nums:{},...this._pi.data};State.saveU(t);UI.toast((i18n.toast_updated||'').replace('{name}',this._qt));this._pi=null;this._qt=null;this.cls();setTimeout(()=>location.reload(),900)}
};
const Missions={
  ...
  open(){if(!this._d)this.init();const b=E('missions-body');b.innerHTML='';const now=new Date();now.setHours(0,0,0,0);const md=new Date(now);md.setDate(md.getDate()+(md.getDay()===0?-6:1-md.getDay()));const frD=i18n.days||[],frM=i18n.months||[];for(let i=0;i<7;i++){const d=new Date(md);d.setDate(md.getDate()+i);const it=d.getTime()===now.getTime(),c=C('div');c.className='mission-day-card'+(it?' today-card':'');c.innerHTML=`<div class="mission-day-label">${frD[i]} ${d.getDate()} ${frM[d.getMonth()]} ${d.getFullYear()} ${it?`<span class="today-badge">${i18n.today_badge||''}</span>`:''}</div>`;for(let m=0;m<3;m++){const r=C('div');r.className='mission-row';const ta=C('textarea');ta.className='mission-input';ta.rows=1;ta.placeholder=(i18n.mission_placeholder||'').replace('{n}',m+1);ta.value=this._d[i].texts[m]||'';ta.oninput=()=>{ta.style.height='auto';ta.style.height=Math.min(ta.scrollHeight,80)+'px';this._d[i].texts[m]=ta.value;this.save()};ta.onfocus=()=>ta.oninput();r.appendChild(ta);c.appendChild(r)}b.appendChild(c)}E('mod-missions').classList.add('open')}
};

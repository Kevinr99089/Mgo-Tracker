...
const Actions={
  ...
  if(!a)return;e.stopPropagation();const act=a.dataset.action,m={
    'toggle-menu':()=>{const p1=E("pop-menu"),p2=E("pop-players");if(p1.classList.contains("show")){p1.classList.remove("show")}else{p2.classList.remove("show");p1.classList.add("show");PlayerManager.close()}},
    'toggle-players':()=>{const p1=E("pop-menu"),p2=E("pop-players");if(p2.classList.contains("show")){p2.classList.remove("show");PlayerManager.close()}else{p1.classList.remove("show");p2.classList.add("show")}},
    'open-players':()=>PlayerManager.open(),'cancel-players':()=>{if(E("setup-mod").classList.contains("open")){E("pop-players").classList.remove("show");setTimeout(()=>PlayerManager.close(),250)}else{PlayerManager.close()}},'add-player':()=>PlayerManager.add(),'save-players':()=>PlayerManager.save(),
    ...
    'save-file':()=>{const v="4.2.0 (Web)",b=new Blob([JSON.stringify({version:v,config:State.cfg,users:State.usr})],{type:"application/json"}),u=URL.createObjectURL(b),lk=C("a");lk.href=u;lk.download=`Mgo_Backup_V${v}.json`;D.body.appendChild(lk);lk.click();D.body.removeChild(lk);setTimeout(()=>URL.revokeObjectURL(u),5e3);UI.toast(T("file_dl"))},

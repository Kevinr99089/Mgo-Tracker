...
const Missions={
  ...
  init(){const _df=()=>Array.from({length:7},()=>({texts:['','','']}));const k=this.gk(new Date()),sk=LS.getItem('mgo_missions_week');if(sk!==k){const ahead=LS.getItem('mgo_missions_data_'+k);try{this._d=ahead?JSON.parse(ahead)||_df():_df()}catch(e){this._d=_df()}LS.setItem('mgo_missions_week',k);LS.setItem('mgo_missions_data',JSON.stringify(this._d))}else{try{this._d=JSON.parse(LS.getItem('mgo_missions_data'))||_df()}catch(e){this._d=_df()}}},
  save(){const s=JSON.stringify(this._d);LS.setItem('mgo_missions_data',s);LS.setItem('mgo_missions_data_'+this.gk(new Date()),s)},
